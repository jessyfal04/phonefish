...
func _ready() -> void:
	...
	($"HTTPRequest" as HTTPRequest).request("https://jessyfal04.dev/api/phonefish/scores/register?pseudo=%s&identifier=%s&token=%s&score=%d" % [connection.pseudo, connection.identifier, connection.token, game.score], [],  HTTPClient.METHOD_POST);
